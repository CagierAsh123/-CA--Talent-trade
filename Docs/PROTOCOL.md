...
| `mpaid` | MarketPaid | 买家→卖家 | `PHXTT\|v1\|mpaid\|listingId\|buyerUuid` | 确认付款 |
...
| `tacc` | TradeAccept | B→A | `PHXTT\|v1\|tacc\|tradeId\|targetUuid\|b64TargetName` | 接受交易 |
| `trej` | TradeReject | B→A | `PHXTT\|v1\|trej\|tradeId\|targetUuid` | 拒绝交易 |
...
| `texe` | TradeExecute | 双方 | `PHXTT\|v1\|texe\|tradeId\|senderUuid\|b64CompressedPawnData` | 执行交易 (发送 Pawn) |
...
| `rexp` | RentalExpiry | 系统/承租方 | `PHXTT\|v1\|rexp\|rentalId\|renterUuid\|b64CompressedPawnData` | 到期自动归还 |
| `rdead` | RentalDead | 承租方→出租方 | `PHXTT\|v1\|rdead\|rentalId\|renterUuid` | Pawn 死亡通知 |
| `rrev` | RentalRevive | 出租方→承租方 | `PHXTT\|v1\|rrev\|rentalId\|ownerUuid` | 复活确认 |

### Def 兼容性交换

| 类型码 | 枚举值 | 方向 | 格式 | 说明 |
|--------|--------|------|------|------|
| `defs` | DefManifest | A→B | `PHXTT\|v1\|defs\|requestId\|senderUuid\|targetUuid\|b64DefList` | 发送本地 Def 清单 |
| `dack` | DefAck | B→A | `PHXTT\|v1\|dack\|requestId\|senderUuid\|b64MissingDefs` | 回复缺失的 Def 列表 |
